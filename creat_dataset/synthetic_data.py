...
class OCRData(object):
    ...
    def setArguments(self):
        self.args['characters_length_tuple'] = (10, 16)
        ...
        self.args['font_size_min'] = 60
        self.args['image_number'] = 10
        self.args['save_full_image'] = 1
        self.args['add_rectangle'] = 1
        return self.args
    ...
    def synthesizeAllImages(self, image_number):
        ...
        for i in tqdm.tqdm(range(0, image_number)):
            ...
            print('正在使用背景' + str(background_image_path))
 ...
 #需要修改标注信息保存的格式：图片路径 文字内容
    def saveAnnotation(self, content_index, points, image_index):
        for index, one_content in enumerate(content_index):
            ...
            with codecs.open(ann_path, 'w', encoding='utf-8') as file:
                to_str = str(one_content)
                to_single_number = to_str.replace('[', '').replace(']', '').replace(',', '')
                #print(test)
                file.write(ann_name.split('.')[0] + '.jpg ' + to_single_number)
            
    def putContent2Image(self, mulcontents, background_image_path, font_path, add_rectangle=0, resize_rate=1):
        try:
            image = Image.open(background_image_path)
            mulcontents_points = []
            font_size_max = image.size[0]/self.args['characters_length_tuple'][1]
            #print(font_size_max,self.args['font_size_min'])
            ...
            left_center_point= (random.randint(0, image.size[0]-font_size*max([len(i) for i in mulcontents])), random.randint(font_size*len(mulcontents), image.size[1]-font_size*len(mulcontents)/1))
    ...
    def chooseSaveDirByIndex(self, image_number_index, is_part_img=1):
