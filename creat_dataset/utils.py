...
#添加了一个Distortion，可以考虑用GaussianDistortion
def augmentImage(txt_img, points):
    # Augment rate for eatch type
    rot = 0
    gaussian_distortion_rate = random.uniform(0, 1)
    if rot < 1:
        distort = aug.Distort(1.0, 20, 20, 4)
...
def get_contents(id_character_dict, length_range_tuple, line_number=1):
...
def mergeBgimgAndTxtimgPoints(left_center, points):
    ...
    for k in range(0, 4):
        #print(type(points),type(np.array([left, top])))
        points[k, :] += np.array([left, top]).astype('int32')
    return points
